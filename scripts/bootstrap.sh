...
parse_dry_run "$@"
require_config KUBERNETES_MINOR KUBERNETES_VERSION CONTROL_PLANE_ENDPOINT POD_CIDR CNI_MANIFEST_URL
...
if [[ "$DRY_RUN" == true ]]; then
  ...
  if [[ "$NODE_ROLE" == control-plane ]]; then
    log "Would initialize the API endpoint ${CONTROL_PLANE_ENDPOINT}, install the Calico CNI, and create join credentials."
  else
